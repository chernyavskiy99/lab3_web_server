...
app.get('/weather/city', (req, res) => {
	request.get(`${endpoint}?q=${req.query.q}&appid=${token}`, (err, response, body) => {
		return formRes(res, err, body);
	});
});	

app.get('/weather/coordinates', (req, res) => {
	request.get(`${endpoint}?lat=${req.query.lat}&lon=${req.query.lon}&appid=${token}`, (err, response, body) => {
		return formRes(res, err, body);
	});
});
